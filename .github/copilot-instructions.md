...
**Key commands:** `/ars-lit-review`, `/ars-3w`, `/ars-outline`, `/ars-full`, `/ars-reviewer`, `/ars-rebuttal-audit`, `/ars-citation-check`
...
⚠️ **Permissions:** keep Copilot CLI's normal permission prompts enabled. The extension adds plugin-infrastructure protection through `onPreToolUse`, but Copilot hooks do not expose subagent identity, so the upstream per-agent Bucket A fence is unavailable. Test full pipeline runs in a separate Copilot CLI session.
