...
- A configured Copilot subscription or BYOK provider (`COPILOT_PROVIDER_*`)
- *Optional:* Pandoc for DOCX, tectonic + Source Han Serif TC for APA 7.0 PDF (Markdown output works without either)
- *Optional (real Python):* The core skills are prompt-driven. A **real Python interpreter** is needed only for the optional write-scope guard and features that invoke Python (revision patches, submission verification, cache invalidation, and mark/unmark-read). The Copilot runtime probes `python3`, `python`, and Windows `py -3`, rejects empty Microsoft Store stubs, and degrades silently if none works.
...
**👉 [docs/SETUP.md](docs/SETUP.md)** — full Copilot setup guide plus clearly labeled reference methods for other platforms, optional Pandoc/tectonic output, and cross-model verification (`ARS_CROSS_MODEL`).
...
**👉 [docs/PERFORMANCE.md](docs/PERFORMANCE.md)** — per-mode token budgets, full-pipeline estimate (~$4–6 for a 15k-word paper), Copilot provider guidance, and optional model tiering.
